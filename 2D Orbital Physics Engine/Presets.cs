...
namespace _2D_Orbital_Physics_Engine
{
    public static class Presets
    {
        ...
        public static void SpawnCompleteSolarSystem()
        {
            ...
            SharedData.bodies.Add(SharedData.CreaetBody(0, 0, SharedData.SolarMass, new Vector(), "Sun"));
            ...
            SharedData.bodies.Add(SharedData.CreaetBody(posMer.X, posMer.Y, SharedData.MercuryMass, velMer, Color.Gray, "Mercury"));

            var (posVen, velVen) = OrbitalElements(SharedData.SolarMass, 0.723 * SharedData.AU, 6.755e-3, 5.51e1, 1.284e2, new Vector(), new Vector());
            SharedData.bodies.Add(SharedData.CreaetBody(posVen.X, posVen.Y, SharedData.VenusMass, velVen, Color.Yellow, "Venus"));

            //////////////////////////////////////////
            /// Earth System
            var (posEar, velEar) = OrbitalElements(SharedData.SolarMass, SharedData.AU, 1.61e-2, 3.02e2, 4.67e1, new Vector(), new Vector());
            SharedData.bodies.Add(SharedData.CreaetBody(posEar.X, posEar.Y, SharedData.EarthMass, velEar, Color.SkyBlue, "Earth"));

            var (posMoo, velMoo) = OrbitalElements(SharedData.EarthMass, 2.567e-3 * SharedData.AU, 3.47e-2, 9.81e1, 4.317e1, posEar, velEar);
            SharedData.bodies.Add(SharedData.CreaetBody(posMoo.X, posMoo.Y, SharedData.MoonMass, velMoo, Color.LightGray, "Moon"));

            //////////////////////////////////////////
            /// Mars System
            var (posMar, velMar) = OrbitalElements(SharedData.SolarMass, 1.524 * SharedData.AU, 9.33e-2, 2.87e2, 1.423e2, new Vector(), new Vector());
            SharedData.bodies.Add(SharedData.CreaetBody(posMar.X, posMar.Y, SharedData.MarsMass, velMar, Color.Red, "Mars"));

            var (posPho, velPho) = OrbitalElements(SharedData.MarsMass, 6.27e-5 * SharedData.AU, 1.5e-2, 2.01e2, 1.22e2, posMar, velMar);
            SharedData.bodies.Add(SharedData.CreaetBody(posPho.X, posPho.Y, SharedData.PhobosMass, velPho, Color.Orange, "Phobos"));

            var (posDei, velDei) = OrbitalElements(SharedData.MarsMass, 1.57e-4 * SharedData.AU, 3.17e-4, 2.58e2, 3.045e1, posMar, velMar);
            SharedData.bodies.Add(SharedData.CreaetBody(posDei.X, posDei.Y, SharedData.DeimosMass, velDei, Color.DarkOrange, "Deimos"));
            ...
            SharedData.bodies.Add(SharedData.CreaetBody(posHal.X, posHal.Y, SharedData.HalleyMass, velHal, Color.DarkCyan, "Halley's Comet"));
            ...
            SharedData.bodies.Add(SharedData.CreaetBody(posBpp.X, posBpp.Y, SharedData.HaleBoppMass, velBpp, Color.LightCyan, "Comet Hale-Bopp"));
            ...
            SharedData.bodies.Add(SharedData.CreaetBody(posEnck.X, posEnck.Y, SharedData.EnckeMass, velEnck, Color.Cyan, "Comet Encke"));

            var (posCer, velCer) = OrbitalElements(SharedData.SolarMass, 2.76 * SharedData.AU, 7.96e-2, 7.3e1, 2.74e2, new Vector(), new Vector());
            SharedData.bodies.Add(SharedData.CreaetBody(posCer.X, posCer.Y, SharedData.CeresMass, velCer, Color.SlateGray, "Ceres"));

            var (posVes, velVes) = OrbitalElements(SharedData.SolarMass, 2.36 * SharedData.AU, 8.92e-2, 1.5e2, 7.7e1, new Vector(), new Vector());
            SharedData.bodies.Add(SharedData.CreaetBody(posVes.X, posVes.Y, SharedData.VestaMass, velVes, Color.Gray, "Vesta"));
            ...
            SharedData.bodies.Add(SharedData.CreaetBody(posPal.X, posPal.Y, SharedData.PallasMass, velPal, Color.Gray, "Pallas"));
            ...
            SharedData.bodies.Add(SharedData.CreaetBody(posHyg.X, posHyg.Y, SharedData.HygieaMass, velHyg, Color.SlateGray, "Hygiea"));
            ...
            SharedData.bodies.Add(SharedData.CreaetBody(posEro.X, posEro.Y, SharedData.ErosMass, velEro, Color.LightGray, "Eros"));

            //////////////////////////////////////////
            /// Jupiter System
            var (posJup, velJup) = OrbitalElements(SharedData.SolarMass, 5.2 * SharedData.AU, 4.9e-2, 2.74e2, 2.61e2, new Vector(), new Vector());
            SharedData.bodies.Add(SharedData.CreaetBody(posJup.X, posJup.Y, SharedData.JupiterMass, velJup, Color.Beige, "Jupiter"));

            var (posIo, velIo) = OrbitalElements(SharedData.JupiterMass, 2.821e-3 * SharedData.AU, 3.555e-3, 3.246e1, 1.89e2, posJup, velJup);
            SharedData.bodies.Add(SharedData.CreaetBody(posIo.X, posIo.Y, SharedData.IoMass, velIo, Color.GreenYellow, "Io"));

            var (posEur, velEur) = OrbitalElements(SharedData.JupiterMass, 4.487e-3 * SharedData.AU, 9.418e-3, 2.046e2, 9.077e1, posJup, velJup);
            SharedData.bodies.Add(SharedData.CreaetBody(posEur.X, posEur.Y, SharedData.EuropaMass, velEur, Color.Wheat, "Europa"));

            var (posGan, velGan) = OrbitalElements(SharedData.JupiterMass, 7.158e-3 * SharedData.AU, 2.157e-3, 3.206e2, 2.955e2, posJup, velJup);
            SharedData.bodies.Add(SharedData.CreaetBody(posGan.X, posGan.Y, SharedData.GanymedeMass, velGan, Color.LightGray, "Ganymede"));

            var (posCal, velCal) = OrbitalElements(SharedData.JupiterMass, 1.258e-2 * SharedData.AU, 7.112e-3, 1.945e1, 5.847e1, posJup, velJup);
            SharedData.bodies.Add(SharedData.CreaetBody(posCal.X, posCal.Y, SharedData.CallistoMass, velCal, Color.LightYellow, "Callisto"));
            ...
            SharedData.bodies.Add(SharedData.CreaetBody(posSat.X, posSat.Y, SharedData.SaturnMass, velSat, Color.BurlyWood, "Saturn", true));

            var (posEnc, velEnc) = OrbitalElements(SharedData.SaturnMass, 1.593e-3 * SharedData.AU, 3.524e-3, 6.731e1, 1.374e2, posSat, velSat);
            SharedData.bodies.Add(SharedData.CreaetBody(posEnc.X, posEnc.Y, SharedData.EnceladusMass, velEnc, Color.LightCyan, "Enceladus"));

            var (posTit, velTit) = OrbitalElements(SharedData.SaturnMass, 8.168e-3 * SharedData.AU, 2.875e-2, 1.684e2, 2.389e2, posSat, velSat);
            SharedData.bodies.Add(SharedData.CreaetBody(posTit.X, posTit.Y, SharedData.TitanMass, velTit, Color.LightYellow, "Titan"));

            var (posMim, velMim) = OrbitalElements(SharedData.SaturnMass, 1.243e-3 * SharedData.AU, 1.914e-2, 1.885e2, 2.585e2, posSat, velSat);
            SharedData.bodies.Add(SharedData.CreaetBody(posMim.X, posMim.Y, SharedData.MimasMass, velMim, Color.LightGray, "Mimas"));

            var (posIap, velIap) = OrbitalElements(SharedData.SaturnMass, 2.378e-2 * SharedData.AU, 2.803e-2, 2.287e2, 4.879, posSat, velSat);
            SharedData.bodies.Add(SharedData.CreaetBody(posIap.X, posIap.Y, SharedData.IapetusMass, velIap, Color.SandyBrown, "Iapetus"));

            var (posRhe, velRhe) = OrbitalElements(SharedData.SaturnMass, 3.524e-3 * SharedData.AU, 1.102e-3, 1.721e2, 3.201e2, posSat, velSat);
            SharedData.bodies.Add(SharedData.CreaetBody(posRhe.X, posRhe.Y, SharedData.RheaMass, velRhe, Color.Gray, "Rhea"));

            var (posDio, velDio) = OrbitalElements(SharedData.SaturnMass, 2.524e-3 * SharedData.AU, 2.557e-3, 8.053e1, 4.963e1, posSat, velSat);
            SharedData.bodies.Add(SharedData.CreaetBody(posDio.X, posDio.Y, SharedData.DioneMass, velDio, Color.SlateGray, "Dione"));
            ...
            SharedData.bodies.Add(SharedData.CreaetBody(posUra.X, posUra.Y, SharedData.UranusMass, velUra, Color.Cyan, "Uranus", false, false, true));

            var (posTita, velTita) = OrbitalElements(SharedData.UranusMass, 2.916e-3 * SharedData.AU, 2.449e-3, 2.541e2, 5.564e1, posUra, velUra);
            SharedData.bodies.Add(SharedData.CreaetBody(posTita.X, posTita.Y, SharedData.TitaniaMass, velTita, Color.LightSlateGray, "Titania"));

            var (posObe, velObe) = OrbitalElements(SharedData.UranusMass, 3.900e-3 * SharedData.AU, 1.910e-3, 1.544e2, 3.605e1, posUra, velUra);
            SharedData.bodies.Add(SharedData.CreaetBody(posObe.X, posObe.Y, SharedData.OberonMass, velObe, Color.LightSalmon, "Oberon"));

            var (posUmb, velUmb) = OrbitalElements(SharedData.UranusMass, 1.777e-3 * SharedData.AU, 3.290e-3, 5.023, 7.865e1, posUra, velUra);
            SharedData.bodies.Add(SharedData.CreaetBody(posUmb.X, posUmb.Y, SharedData.UmbrielMass, velUmb, Color.DarkGray, "Umbriel"));

            var (posAri, velAri) = OrbitalElements(SharedData.UranusMass, 1.276e-3 * SharedData.AU, 1.163e-3, 8.662e1, 1.819e2, posUra, velUra);
            SharedData.bodies.Add(SharedData.CreaetBody(posAri.X, posAri.Y, SharedData.ArielMass, velAri, Color.Gray, "Ariel"));

            var (posMir, velMir) = OrbitalElements(SharedData.UranusMass, 8.682e-4 * SharedData.AU, 1.288e-3, 5.019e1, 2.164e2, posUra, velUra);
            SharedData.bodies.Add(SharedData.CreaetBody(posMir.X, posMir.Y, SharedData.MirandaMass, velMir, Color.LightGray, "Miranda"));
            ...
            SharedData.bodies.Add(SharedData.CreaetBody(posNep.X, posNep.Y, SharedData.NeptuneMass, velNep, Color.Blue, "Neptune", false, false, false, true));

            var (posTri, velTri) = OrbitalElements(SharedData.NeptuneMass, 2.371e-3 * SharedData.AU, 2.777e-5, 2.060e2, 3.337e2, posNep, velNep);
            SharedData.bodies.Add(SharedData.CreaetBody(posTri.X, posTri.Y, SharedData.TritonMass, velTri, Color.LightGray, "Triton"));

            var (posPro, velPro) = OrbitalElements(SharedData.NeptuneMass, 7.866e-4 * SharedData.AU, 7.078e-4, 2.160e2, 3.476e2, posNep, velNep);
            SharedData.bodies.Add(SharedData.CreaetBody(posPro.X, posPro.Y, SharedData.ProteusMass, velPro, Color.Gray, "Proteus"));

            var (posNer, velNer) = OrbitalElements(SharedData.NeptuneMass, 3.688e-2 * SharedData.AU, 7.471e-1, 2.973e2, 2.166e2, posNep, velNep);
            SharedData.bodies.Add(SharedData.CreaetBody(posNer.X, posNer.Y, SharedData.NereidMass, velNer, Color.DarkGray, "Nereid"));
            ...
            SharedData.bodies.Add(SharedData.CreaetBody(posPlu.X, posPlu.Y, SharedData.PlutoMass, velPlu, Color.DarkOrange, "Pluto"));

            var (posCha, velCha) = OrbitalElements(SharedData.PlutoMass, 1.309e-4 * SharedData.AU, 1.605e-4, 1.726e2, 1.171e2, posPlu, velPlu);
            SharedData.bodies.Add(SharedData.CreaetBody(posCha.X, posCha.Y, SharedData.CharonMass, velCha, Color.LightGray, "Charon"));

            var (posNix, velNix) = OrbitalElements(SharedData.PlutoMass, 8.480e-4 * SharedData.AU, 5.989e-1, 2.838e2, 3.562e2, posPlu, velPlu);
            SharedData.bodies.Add(SharedData.CreaetBody(posNix.X, posNix.Y, SharedData.NixMass, velNix, Color.DarkSlateGray, "Nix"));

            var (posHyd, velHyd) = OrbitalElements(SharedData.PlutoMass, 3.614e-4 * SharedData.AU, 2.062e-1, 3.038e2, 2.110e2, posPlu, velPlu);
            SharedData.bodies.Add(SharedData.CreaetBody(posHyd.X, posHyd.Y, SharedData.HydraMass, velHyd, Color.SlateGray, "Hydra"));

            var (posKer, velKer) = OrbitalElements(SharedData.PlutoMass, 3.967e-4 * SharedData.AU, 1.505e-1, 3.119e2, 8.317e1, posPlu, velPlu);
            SharedData.bodies.Add(SharedData.CreaetBody(posKer.X, posKer.Y, SharedData.KerberosMass, velKer, Color.DarkGray, "Kerberos"));

            var (posSty, velSty) = OrbitalElements(SharedData.PlutoMass, 6.711e-4 * SharedData.AU, 5.566e-1, 3.011e2, 8.015, posPlu, velPlu);
            SharedData.bodies.Add(SharedData.CreaetBody(posSty.X, posSty.Y, SharedData.StyxMass, velSty, Color.LightGray, "Styx"));
            ...
            SharedData.bodies.Add(SharedData.CreaetBody(posHau.X, posHau.Y, SharedData.HaumeaMass, velHau, Color.RosyBrown, "Haumea"));
            ...
            SharedData.bodies.Add(SharedData.CreaetBody(posMak.X, posMak.Y, SharedData.MakemakeMass, velMak, Color.PaleVioletRed, "Makemake"));
            ...
            SharedData.bodies.Add(SharedData.CreaetBody(posEri.X, posEri.Y, SharedData.ErisMass, velEri, Color.GhostWhite, "Eris"));
            ...
            SharedData.bodies.Add(SharedData.CreaetBody(posSed.X, posSed.Y, SharedData.SednaMass, velSed, Color.RosyBrown, "Sedna"));

            var (posVoy1, velVoy1) = OrbitalElements(SharedData.SolarMass, -3.217 * SharedData.AU, 3.749, 3.384e2, 9.923e1, new Vector(), new Vector());
            SharedData.bodies.Add(SharedData.CreaetBody(posVoy1.X, posVoy1.Y, SharedData.Voyager1Mass, velVoy1, Color.Gold, "Voayger 1"));

            var (posVoy2, velVoy2) = OrbitalElements(SharedData.SolarMass, -4.017 * SharedData.AU, 6.288, 1.300e2, 8.251e1, new Vector(), new Vector());
            SharedData.bodies.Add(SharedData.CreaetBody(posVoy2.X, posVoy2.Y, SharedData.Voyager2Mass, velVoy2, Color.Gold, "Voayger 2"));
        }
        ...
        public static void SpawnEarthSystem()
        {
            ...
            SharedData.bodies.Add(SharedData.CreaetBody(0, 0, SharedData.EarthMass, new Vector(), Color.SkyBlue, "Earth"));
        ...
        public static void SpawnMarsSystem()
        {
            ...
            SharedData.bodies.Add(SharedData.CreaetBody(0, 0, SharedData.MarsMass, new Vector(), Color.Red, "Mars"));
        ...
        public static void SpawnJupiterSystem()
        {
            ...
            SharedData.bodies.Add(SharedData.CreaetBody(0, 0, SharedData.JupiterMass, new Vector(), Color.Beige, "Jupiter"));
        ...
        public static void SpawnSaturnSystem()
        {
            ...
            SharedData.bodies.Add(SharedData.CreaetBody(0, 0, SharedData.SaturnMass, new Vector(), Color.BurlyWood, "Saturn", true));
            ...
            SharedData.bodies.Add(SharedData.CreaetBody(posRhe.X, posRhe.Y, SharedData.RheaMass, velRhe, Color.LightSlateGray, "Rhea"));
        ...
        public static void SpawnUranusSystem()
        {
            ...
            SharedData.bodies.Add(SharedData.CreaetBody(0, 0, SharedData.UranusMass, new Vector(), Color.Cyan, "Uranus", false, false, true));
        ...
        public static void SpawnNeptuneSystem()
        {
            ...
            SharedData.bodies.Add(SharedData.CreaetBody(0, 0, SharedData.NeptuneMass, new Vector(), Color.Blue, "Neptune", false, false, false, true));
        ...
        public static void SpawnPlutoSystem()
        {
            ...
            SharedData.bodies.Add(SharedData.CreaetBody(0, 0, SharedData.PlutoMass, new Vector(), Color.DarkOrange, "Pluto"));
